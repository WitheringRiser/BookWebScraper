...
def COMBINED_FUNC_get_bookfinder_results(list_of_titles, country_code):
	book_finder_results = BF_setup_book_finder_results(list_of_titles)
...
def BF_setup_book_finder_results(list_of_titles):
	book_finder_results = {}
	'''
	blank_title_information = {
	'Cheapest US Price:': 0
	'Cheapest UK Price:': 0
	'Cheapest US Amazon Price:': 0
	}
	'''
	for title in list_of_titles:
	...
	for title in titles_links_volumes:
		...
		for link in links:
			for i in range(2):
				...
				if i == 1:
					soup = BF_fetch_refreshed_html_contents_of_sublink(link)
				...
				if i == 0:
					version1_title_sublink_info = title_sublink_info.copy()
					title_sublink_info = BF_create_empty_title_sublink_info_dict()
				if i == 1:
					title_sublink_info = BF_keep_best_results(version1_title_sublink_info, title_sublink_info)
					print(title_sublink_info)
			results_all_subpages[title] = title_sublink_info
			title_sublink_info = BF_create_empty_title_sublink_info_dict()
...
def BF_keep_best_results(dict_version1, dict_version2):
	best_version = BF_create_empty_title_sublink_info_dict()

	cheapest_price_version1 = dict_version1['Cheapest price'][0]
	cheapest_price_version2 = dict_version2['Cheapest price'][0]
	if cheapest_price_version1 <= cheapest_price_version2:
		best_version['Cheapest price'].append(cheapest_price_version1)
		best_version['Cheapest store'].append(dict_version1['Cheapest store'][0])
	else:
		best_version['Cheapest price'].append(cheapest_price_version2)
		best_version['Cheapest store'].append(dict_version2['Cheapest store'][0])

	cheapest_specific_store_price_version1 = dict_version1['Cheapest Amazon price'][0]
	cheapest_specific_store_price_version2 = dict_version2['Cheapest Amazon price'][0]
	if cheapest_specific_store_price_version1 <= cheapest_specific_store_price_version2:
		best_version['Cheapest Amazon price'].append(cheapest_specific_store_price_version1)
		best_version['Cheapest Amazon store'].append(dict_version1['Cheapest Amazon store'][0])
	else:
		best_version['Cheapest Amazon price'].append(cheapest_specific_store_price_version2)
		best_version['Cheapest Amazon store'].append(dict_version2['Cheapest Amazon store'][0])
...
def BF_get_first_in_stock_html_contents(soup):
	...
	if 'out of stock' not in first_entry_all_text:
	...
	while not in_stock_entry_found and not checked_all_results:
		...
		if 'out of stock' not in current_entry_all_text:
			in_stock_entry_found = True
			continue
		i += 1
